...
Description:
Practice creating and using a simple Python class with attributes and methods.


Instructions:
Imagine you are writing a program to help a library keep track of their books.

Complete each of these steps:

1. Create a "Book" class to keep track of a single book. The Book class should
   keep track of a book's title, author, ISBN, and number of pages. The Book 
   class should also have these methods: get_description(), get_ISBN(), 
   get_author(), get_title(), get_pages(), and __str__().
...
2. Stretch - Add a dictionary to main() that keeps track of multiple books
   using the ISBN as the key. Add three books to the dictionary,
...
def main():
    ...
    # Use print(book1) to call the __str__ method and show the book nicely formatted
    print(book1)


    # TODO: Create a dictionary of books with ISBN as the key
    books_dict = {}
    books_dict[book1.ISBN] = book1
    books_dict[book2.ISBN] = book2
    books_dict[book3.ISBN] = book3
